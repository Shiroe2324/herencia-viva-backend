...
import { REGENERATE_MFA_BACKUP_CODES_DOCS } from '@/auth/docs/constants/auth-mfa.constant';
import { USER_PASSWORD_LENGTH_RANGE } from '@/configs';
import { BACKUP_CODES_EXAMPLE, PASSWORD_EXAMPLE, TOTP_EXAMPLE } from '@/constants';
import { IsPassword, IsToken } from '@/decorators';

const { REQUEST, RESPONSE } = REGENERATE_MFA_BACKUP_CODES_DOCS;

@ApiSchema({ name: REQUEST.NAME, description: REQUEST.DESCRIPTION })
export class RegenerateMfaBackupCodesRequest {
...
export class RegenerateMfaBackupCodesResponse {
